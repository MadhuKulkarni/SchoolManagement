        ...
        if (isset($fileName)) {
            ...
            if (empty($errors)) {
                ...
                if ($didUpload) {
                    ...
                    if($action == 'update') {
                        $query = 'UPDATE student_assignment SET assignment_upload="'.$uploadPath.'" WHERE timetable_id='.$timetable_id.' AND student_id='.$student_id;
                    } else {
                        $query = 'insert into student_assignment (assignment_upload,timetable_id,student_id) values("'.$uploadPath.'",'.$timetable_id.','.$student_id.')'; 
                    }
