              ...
              echo '<section  class="content-header" style="background-color:white; width:71%;border-top: 3px solid #3c8dbc;">';
               ...
               echo '<div class="row" id="table1">';
                 echo '<div class="box-header with-border">';
                   echo  '<h3 class="box-title">Notes</h3>';
                 echo '</div>';
               $sql= "SELECT timetable.time_date,timetable.start_time,timetable.end_time,timetable.notes,
               subject.name          
               from timetable      
               inner join subject
               ...
               while($row=mysqli_fetch_assoc($result2)){
                  ...
                  // print_r($row);
                  // echo "</pre>";
				$notes = ($row['notes'] == '') ? 'Not available': '<a href="'.$row['notes'].'" target="_blank">View Notes</a>';
