   ...
   <?php 
      if($grade!=='' && $subject!==''){
          ...
          echo '<div class="row" id="table1">';
            ...
            echo '</div>';
          $sql= "SELECT timetable.time_date,timetable.start_time,timetable.end_time,timetable.notes,subject.name,timetable.id
          from timetable
          inner JOIN subject
      ...
      echo "<thead> 
          ...
          while($row=mysqli_fetch_assoc($result2)){
              $notes= ($row['notes']=='')? '<input type="file" class="fileAjax" name="fileAjax"/ style="width:200px;"><button class="uploadNotes">Submit</button><span class="status"></span>': '<a href="'.$row['notes'].'" target="_blank">View Notes</a>';
              echo "<tr>
                      <td>{$row['time_date']}</td>
                      <td>{$row['name']}</td>
                      <td>{$row['start_time']}</td>
                      <td>{$row['end_time']}</td>
                      <td style='display:flex' data-id='".$row['id']."'>{$notes}</td>
                    </tr>";
          }
          echo "</tbody>";
         echo "</table>";
...
<script>
   $(document).ready(function(){
      $('.uploadNotes').on('click', function(event) {
